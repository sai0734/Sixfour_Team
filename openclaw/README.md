...
| 주간 브리핑이 `FailoverError: Rate limit reached ... TPM` 에러로 계속 실패 (일간체크는 멀쩡한데 이것만) | 일회성 사고가 아니라 **이 작업 자체가 구조적으로 빠듯함**. 회원(최대 1000명)/업체/상품/6개월 매출을 다 모은 뒤 리포트 전체(9개 섹션+SVG 차트+카드)를 한 번의 요청으로 통째로 생성하는데, 실측으로 이 마지막 한 번의 요청만 9만 토큰 안팎 필요함. 그 앞서 데이터수집에 이미 10만 토큰 넘게 써놓은 상태라, 이 조직의 분당 한도(20만, gpt-5.4-mini 기준)와 거의 항상 부딪힘. 재시도로는 잘 안 풀림 — 근본 해결은 (a) OpenAI 플랫폼(platform.openai.com/account/rate-limits)에서 이 조직의 TPM 등급을 올리거나, (b) `WEDDING_WEEKLY_BRIEFING.md`를 리포트를 여러 번의 작은 요청으로 나눠 생성하도록 재설계하는 것. 둘 다 아직 미착수 |
